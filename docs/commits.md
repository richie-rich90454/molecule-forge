...
Scopes name the area touched: `chem`, `sim`, `render`, `ui`, `presets`, `audio`, `state`, `wasm`, `scripts`, `deps`, `docs`, `app`.
...
- Generated listings (`docs/molecules.md`) regenerate from `bun run bake` and commit alone.
- Rust changes verify with `cargo test` before committing; TypeScript changes verify with `bun run test` for affected areas.
...
test(wasm): cover force calculator signs
```
